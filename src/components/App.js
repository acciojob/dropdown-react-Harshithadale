...
function App() {
  ...
  const [selState, setSelStates] = useState(null);
  const [selCity, setSelCity] = useState(null);
  const [selLandMark, setSelLandMark] = useState(null);
  ...
  useEffect(() => {
    if (landmark && landmarks.length > 0) {
      const selectedLandmark = landmarks.find((item) => item.name === landmark);
      setSelLandMark(selectedLandmark);
    } else {
      setSelLandMark(null);
    }
  }, [landmark, landmarks]);
